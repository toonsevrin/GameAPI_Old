...
/**
 * <p>
 * A utility class for handling converting an inventory to/from a String to be stored into the MySQL
 * database or any sort of file.
 * </p>
 */
public class InventorySerializer {
    ...
    /**
     * <p>
     * Serializes the player's inventory into a string.
     * </p>
     *
     * @param inventory
    ...
    public static String serialize( PlayerInventory inventory ) {
        ...
        for ( int i = 0; i < content.length; i++ ) {
            InventorySerializer.serialize( content[ i ], sb, "i" + i );
        }
        
        // Armor
        content = inventory.getArmorContents();
        for ( int i = 0; i < content.length; i++ ) {
            InventorySerializer.serialize( content[ i ], sb, "a" + i );
        }
    ...
    /**
     * <p>
     * Deserializes the inventory from the string
     * </p>
     * <p>
     * The current contents of the inventory will be cleared before the deserialized items are added
     * to it.
     * </p>
     *
     * @param inv
     *            inventory to fill with the result
     * @param dataString
     *            inventory serialized string to deserialize
     */
    public static void deserialize( PlayerInventory inv, String dataString ) {
        ...
        String[] data = dataString.split( InventorySerializer.M_SEP );
        for ( int i = 0; i < data.length; i += 7 ) {
            
            // Deserialize the data
            String key = data[ i ];
            ItemStack item = InventorySerializer.deserialize( data, i );
    ...
    /**
     * <p>
     * Deserializes the armor contents from the inventory only.
     * </p>
     * <p>
     * The main inventory contents will be ignored.
     * </p>
     * <p>
     * If an armor piece was not in the data, it will be null instead.
     * </p>
     * <p>
     * The returned array is in the same order as a player inventory's armor array.
     * </p>
     *
     * @param dataString
     *            inventory serialized string to deserialize
    ...
    private static void serialize( ItemStack item, StringBuilder sb, String id ) {
        ...
        // Serialize the item
        sb.append( id );
        sb.append( InventorySerializer.M_SEP );
        ...
        if ( enchants.size() > 0 ) {
            for ( Map.Entry<Enchantment, Integer> enchant : enchants.entrySet() ) {
                ...
                sb.append( InventorySerializer.S_SEP );
    ...
    private static ItemStack deserialize( String[] data, int start ) {
        ...
        if ( data.length > start + 4 && data[ start + 4 ].length() > 0 ) {
            String[] enchantData = data[ start + 4 ].split( InventorySerializer.S_SEP );
        ...
        if ( data.length > start + 5 && data[ start + 5 ].length() > 0 ) {
            meta.setDisplayName( data[ start + 5 ] );
        }
        if ( data.length > start + 6 && data[ start + 6 ].length() > 0 ) {
            List<String> lore = new ArrayList<String>();
            int index = 0;
            while ( index < data[ start + 6 ].length() ) {
                int next = data[ start + 6 ].indexOf( InventorySerializer.S_SEP, index );
                int length = Integer.parseInt( data[ start + 6 ].substring( index, next ) );
                index = next + InventorySerializer.S_SEP.length() + length;
                if ( index > data[ start + 6 ].length() ) {
                    index = data[ start + 6 ].length();
                }
                lore.add( length == 0 ? "" : data[ start + 6 ].substring( next + InventorySerializer.S_SEP.length(), index ) );
            }
            meta.setLore( lore );
        }
