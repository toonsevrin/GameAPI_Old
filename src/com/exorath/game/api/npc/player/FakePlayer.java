...
public class FakePlayer {
    ...
    public Object spawnPlayer(Location loc) {
        Object nms_DedicatedPlayerList = Reflection.getMethod(NMS.get().getBukkitServerClass(), "getHandle")
        ...
        Object nms_Player = Reflection.getConstructor(NMS.get().getPlayerClass(), NMS.get().getMinecraftServerClass(),
                NMS.get().getWorldServerClass(), NMS.get().getGameProfileClass(),
                NMS.get().getPlayerInteractManagerClass())
                .construct(nms_DedicatedServer, nms_WorldServer, nms_GameProfile,
                        NMS.get().createPlayerInteractManager(loc.getWorld()));
        Object nms_NetworkManager = NMS.get().createNetworkManager();
        Object nms_NetServerHandler = NMS.get().createNetServerHandler(nms_DedicatedServer, nms_NetworkManager,
                nms_Player);
        nms_NetServerHandler.getClass();// using it because I don't want to delete this bit

        Reflection.getMethod(NMS.get().getPlayerClass(), "spawnIn", NMS.get().getWorldClass()).invoke(nms_Player,
                nms_WorldServer);
        Object nms_PlayerInteractManager = Reflection.getField(NMS.get().getPlayerClass(), "playerInteractManager")
                .get(nms_Player);
        Reflection.getMethod(NMS.get().getPlayerInteractManagerClass(), "a", NMS.get().getWorldServerClass())
                .invoke(nms_PlayerInteractManager,
                        Reflection.getField(NMS.get().getPlayerClass(), "world").get(nms_Player));// pim.a(world);
        Object nms_WorldData = Reflection.getMethod(NMS.get().getWorldServerClass(), "getData").invoke(nms_WorldServer);
        Object nms_EnumGamemode = Reflection.getMethod(NMS.get().getClass("WorldData"), "getGameType")
                .invoke(nms_WorldData);
        ...
        Reflection.getMethod(NMS.get().getPlayerClass(), "setPositionRotation", double.class, double.class,
                double.class, float.class,
                float.class).invoke(nms_Player, loc.getX(), loc.getY(), loc.getZ(), loc.getYaw(), loc.getPitch());
        Reflection.getMethod(NMS.get().getWorldClass(), "addEntity", NMS.get().getEntityClass()).invoke(nms_WorldServer,
                nms_Player);
        List nms_ListEntityPlayer = (List) Reflection.getField(NMS.get().getClass("PlayerList"), "players")
                .get(nms_DedicatedPlayerList);
        nms_ListEntityPlayer.add(nms_Player);
        Reflection.getMethod(NMS.get().getClass("PlayerList"), "a", NMS.get().getPlayerClass(),
                NMS.get().getWorldServerClass())
                .invoke(nms_DedicatedPlayerList, nms_Player, nms_WorldServer);

        Object nms_ChunkProviderServer = Reflection.getField(NMS.get().getWorldServerClass(), "chunkProviderServer")
                .get(nms_WorldServer);
    ...
    public static Player getBukkitPlayer(Object nms_EntityPlayer) {
        if (NMS.get().getPlayerClass().isAssignableFrom(nms_EntityPlayer.getClass())) {
            return Reflection.getMethod(NMS.get().getPlayerClass(), "getBukkitEntity").invoke(nms_EntityPlayer)
                    .getObjectAs(Player.class);
        }
        return null;
    }
