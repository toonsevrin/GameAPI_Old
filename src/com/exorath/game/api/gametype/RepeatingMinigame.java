 ...
 * Minigames are Games that are short and automatically repeat. Onstop they will wait a while in a lobby (if there's one). If the time is over and there are enough players, start again.
 * Stages:
...
public abstract class RepeatingMinigame extends Minigame {
    ...
    @EventHandler
    public void onPlayerJoin(GPlayerJoinEvent event){
        if(getState() == GameState.WAITING && getPlayerCount() >= getProperties().as(MIN_PLAYERS, Integer.class)){//Enough players to start + waiting to start
            startCountdown();
        }
    }
     ...
     * @param remainingSeconds The amount of seconds remaining
     */
    ...
    private void startFinishing(){
        ...
        for(GamePlayer player : this.getPlayers().getPlayers()){ //Send all players appropriate messages etc.
            sendfinishReward(player);
        }
        Bukkit.getScheduler().scheduleSyncDelayedTask(GameAPI.getInstance(), new Runnable(){ //Start the pregame after FINISHING_TIME seconds
            @Override
            public void run(){
                startPregame();
            }
    ...
    public abstract FinishPacket sendFinishPacket(GamePlayer player);
