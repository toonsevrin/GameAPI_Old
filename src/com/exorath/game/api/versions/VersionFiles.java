...
public class VersionFiles {
    ...
    public VersionFiles(String databaseString){
        ...
        if(databaseString == "") return;
        if(!databaseString.contains(FILE_VERSION_SEPERATOR)) return;
    ...
    public boolean isUpToDate(String fileName, String version){
        if(!versions.containsKey(fileName)) return false; //If file name wasn't loaded from database
        if(!versions.get(fileName).equals(version)) return false; //If versions don't match
